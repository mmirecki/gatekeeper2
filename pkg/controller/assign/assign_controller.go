...
func newReconciler(mgr manager.Manager, cache *mutation.Cache) *AssignReconciler {
	r := &AssignReconciler{cache: cache, Client: mgr.GetClient()}
	return r
}
...
// +kubebuilder:rbac:groups=assign.gatekeeper.sh,resources=*,verbs=get;list;watch;create;update;patch;delete
...
func (r *AssignReconciler) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if !deleted {
		err := r.cache.Insert(mutation.AssignMutator{Assign: m.DeepCopy()})
		if err != nil {
			log.Error(err, "Failed to insert")
		}
	} else {
		err := r.cache.Remove(mutation.AssignMutator{Assign: m.DeepCopy()})
		if err != nil {
